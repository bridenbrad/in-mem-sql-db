 ...
 */
public final class DoubleParser extends AbstractParser {

	public DoubleParser(String nullString) {
		super(nullString);
	}
	...
	public Array parse(String[][] splits, int startRow, int endRow, final int columnIndex) {
		...
		if(nulls == null) {
			for (int i = startRow; i < endRow; i++) {
				result[index++] = Double.parseDouble(splits[i][columnIndex]);
			}	
		} else {
			for (int i = startRow; i < endRow; i++) {
				if(!nulls[i]) {					
		...
		return new NativeArray(new Spec(Type.DOUBLE, result.length, false, getBitmap(nulls)), result);
	}
}
